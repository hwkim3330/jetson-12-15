...
void RSsaem::init_dynamixel_sdk_wrapper(const std::string & usb_port)
{
  ...
  //DynamixelSDKWrapper::Device opencr = {usb_port, 200, 115200, 2.0f};
/*
  ...
  */
  ...
  /*
   dxl_sdk_wrapper_->init_read_memory(
    extern_control_table.millis.addr,
    (extern_control_table.heartbeat.addr - extern_control_table.millis.addr) +
  ...
  /*
   dxl_sdk_wrapper_->init_read_memory(
    ...
    (extern_control_table.bumper_2.addr - extern_control_table.device_status.addr) +
  ...
  /*
   dxl_sdk_wrapper_->init_read_memory(
    extern_control_table.device_status.addr,
    (extern_control_table.heartbeat.addr - extern_control_table.device_status.addr) +
    extern_control_table.heartbeat.length
  ); 
  //*/  
...
void RSsaem::publish_timer(const std::chrono::milliseconds timeout)
{
  publish_timer_ = this->create_wall_timer(
    ...
    [this]() -> void
    {
      ...
      //read to fail dxl_sdk_wrapper_->read_data_set();
      dxl_sdk_wrapper_->read_data_set();
...
void RSsaem::heartbeat_timer(const std::chrono::milliseconds timeout)
{
  heartbeat_timer_ = this->create_wall_timer(
    ...
    [this]() -> void
    {
      ...
      //RCLCPP_INFO(this->get_logger(), "hearbeat count : %d, msg : %s", count, msg.c_str());
    ...
    std::function<void(const geometry_msgs::msg::Twist::SharedPtr)>(
      [this](const geometry_msgs::msg::Twist::SharedPtr msg) -> void
      {
		...
		RCLCPP_INFO( this->get_logger(),
            "cmd_vel_callback : start_addr %d addr_length : %d",
            start_addr, addr_length);
        RCLCPP_INFO( this->get_logger(),
            "cmd_vel_callback : data.dword[0] %d data.dword[5] : %d",
            data.dword[0], data.dword[5]);
