...
using keti::robot::DynamixelSDKWrapper;
...
{
  uint8_t data[2];
  LOG_INFO("DynamixelSDKWrapper", "device_.id (%d)",device_.id);
...
{
  const char * log = NULL;
  //LOG_INFO("DynamixelSDKWrapper", "id[%d] start_addr[%d] ", 
  //   device_.id, read_memory_.start_addr);
  ...
  bool ret = this->read_register(
    ...
    &read_data_buffer_[0],
    &log);
  //LOG_INFO("DynamixelSDKWrapper", "start_addr[%d] length[%d] read_data_buffer[0][%d]", 
  //   read_memory_.start_addr, read_memory_.length, read_data_buffer_[0]);
     
  if (ret == false) {
    LOG_DEBUG("DynamixelSDKWrapper", "Failed to read[%s]", log);
  } else {
...
bool DynamixelSDKWrapper::init_dynamixel_sdk_handlers()
{
  ...
  if (portHandler_->setBaudRate(static_cast<int>(device_.baud_rate))) {
    LOG_INFO("DynamixelSDKWrapper", "Succeeded to change the baudrate! (%d)", device_.baud_rate);
  } else {
